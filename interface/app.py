...
# Chat centralizado estilo ChatGPT + barra de progresso discreta
# Briefing exibido apenas quando o agente declara prontidão
...
# ────────────────────────────────────────────────────────────
# BARRA DE PROGRESSO — discreta, só percentual
# Exibida no topo do chat. Atualiza a cada turno.
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
# BRIEFING FINAL — só renderizado quando agente declara prontidão
# ────────────────────────────────────────────────────────────
...
def _bloco_briefing(demanda) -> str:
    def campo_linha(label, valor, origem=None, turno=None):
        ...
        if origem:
            ...
            <span style="
                font-size: 10px;
                background: {cor_origem}18;
                color: {cor_origem};
                border: 1px solid {cor_origem}44;
                padding: 1px 7px;
                border-radius: 99px;
                margin-left: 6px;
                vertical-align: middle;
            ">{origem.lower()}{turno_txt}</span>"""
        return f"""
        <div style="
            display: flex;
            gap: 12px;
    ...
    tipo_nome = {
        ...
        TipoDemanda.ESTRUTURANTE:  "Estruturante",
        TipoDemanda.PRODUTO_DADOS: "Produto de Dados",
        TipoDemanda.ALARMASTICA:   "Alarmística",
...
def responder(mensagem, historico, sessao_state, modo_str):
    ...
    return (
        ...
        barra_html,
        briefing_html,
        gr.update(visible=pronto),        # seção de briefing
        gr.update(visible=pronto),        # botão aprovar
        gr.update(visible=False),         # arquivo download (reseta)
    )
...
def construir_interface(agente_compilado) -> gr.Blocks:
    ...
    css = """
    /* Centraliza o chat e limita largura para estilo ChatGPT */
    .chat-container {
        max-width: 780px;
        margin: 0 auto;
        width: 100%;
    }
    /* Remove padding lateral excessivo do Gradio em telas largas */
    .gradio-container {
    ...
    with gr.Blocks(title="DataBrief AI", css=css) as demo:
        ...
        # ── Briefing final — aparece só quando pronto ─────────
        with gr.Column(elem_classes=["chat-container"], visible=False) as secao_briefing:
            ...
            arquivo_download = gr.File(
                label="Download do briefing (JSON)",
                visible=False,
                file_types=[".json"],
            )
